...
@use_flow
class SaiThriftHaSanityTest(SaiHelperSimplified):
    ...
    def setUp(self):
        ...
        # We reuse the client class from the active connection to avoid import issues
        try:
        ...
        # Keep track of objects for cleanup on both clients
        self.cleanup_objects = {}  # Map client -> list of objects (or just rely on duplication for now)

    def configureVnet(self, client):
        ...
        status = sai_thrift_create_vip_entry(client, self.vpe,
        ...
        status = sai_thrift_create_direction_lookup_entry(client, self.dle,
        ...
        status = sai_thrift_create_global_trusted_vni_entry(client, self.gtve)
        assert(status == SAI_STATUS_SUCCESS)

        self.in_acl_group_id = sai_thrift_create_dash_acl_group(client,
                                                                ip_addr_family=self.sai_ip_addr_family)
        assert (self.in_acl_group_id != SAI_NULL_OBJECT_ID)
        self.out_acl_group_id = sai_thrift_create_dash_acl_group(client,
                                                                 ip_addr_family=self.sai_ip_addr_family)
        assert (self.out_acl_group_id != SAI_NULL_OBJECT_ID)

        self.vnet = sai_thrift_create_vnet(client, vni=self.vnet_vni)
        assert (self.vnet != SAI_NULL_OBJECT_ID)

        self.outbound_routing_group = sai_thrift_create_outbound_routing_group(client, disabled=False)
        ...
        self.eni = sai_thrift_create_eni(client, cps=10000,
        ...
        self.flow_table = sai_thrift_create_flow_table(client,
                    max_flow_count=128,
        ...
        sai_thrift_set_eni_attribute(client, eni_oid = self.eni, flow_table_id=self.flow_table)

        self.eam = sai_thrift_eni_ether_address_map_entry_t(switch_id=self.switch_id, address = self.eni_mac)
        status = sai_thrift_create_eni_ether_address_map_entry(client,
        ...
        if self.sai_ip_addr_family == SAI_IP_ADDR_FAMILY_IPV4:
            self.out_acl_rule_id = sai_thrift_create_dash_acl_rule(client, dash_acl_group_id=self.out_acl_group_id, priority=10,
        ...
        status = sai_thrift_create_outbound_routing_entry(client, self.ore,
        ...
        status = sai_thrift_create_outbound_ca_to_pa_entry(client, self.ocpe, action=SAI_OUTBOUND_CA_TO_PA_ENTRY_ACTION_SET_TUNNEL_MAPPING,
        ...
        print(f"\n{self.__class__.__name__} configureVnet on client OK\n")
    ...
    def runTest(self):
        ...
        self.configureVnet(self.client)
        
        if self.client_stby:
            print("Configuring Standby Instance...")
            self.configureVnet(self.client_stby)
    ...
    def tearDown(self):
        ...
        # Helper to cleanup a specific client
        def cleanup_client(client, name):
            ...
            try:
                ...
                c_status &= sai_thrift_remove_outbound_ca_to_pa_entry(client, self.ocpe)
                c_status &= sai_thrift_remove_outbound_routing_entry(client, self.ore)
                if self.sai_ip_addr_family == SAI_IP_ADDR_FAMILY_IPV4:
                    c_status &= sai_thrift_remove_dash_acl_rule(client, self.out_acl_rule_id)
                c_status &= sai_thrift_remove_eni_ether_address_map_entry(client, self.eam)
                c_status &= sai_thrift_remove_eni(client, self.eni)
                c_status &= sai_thrift_remove_vnet(client, self.vnet)
                c_status &= sai_thrift_remove_dash_acl_group(client, self.out_acl_group_id)
                c_status &= sai_thrift_remove_dash_acl_group(client, self.in_acl_group_id)
                c_status &= sai_thrift_remove_global_trusted_vni_entry(client, self.gtve)
                c_status &= sai_thrift_remove_direction_lookup_entry(client, self.dle)
                c_status &= sai_thrift_remove_vip_entry(client, self.vpe)
               
            except Exception as e:
                print(f"Cleanup error on {name}: {e}")
            return c_status

        # Cleanup Standby first 
        if self.client_stby:
            cleanup_client(self.client_stby, "Standby")
            try:
        ...
        # Cleanup Active
        cleanup_client(self.client, "Active")
