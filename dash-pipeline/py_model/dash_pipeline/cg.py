...
class MultiFileCallGraph:
    ...
    def _flatten_graph(self, node, prefix=""):
        """Turn nested call graph dict into flat dot-separated chains.
        Only keep chains whose last element is a function (method or global)."""
        results = []
...
def generate_callgraph(project_dir: str, entry_file: str = "dash_pipeline.py") -> List[str]:
...
if __name__ == "__main__":
    project_dir = "./pmv2"
    chains = generate_callgraph(project_dir, entry_file="dash_pipeline.py")

    print("Filtered, cleaned, deduplicated call graph:")
