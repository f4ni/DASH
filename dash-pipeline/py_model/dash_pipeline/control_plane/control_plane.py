...
def resolve_action_name(name: str, ctx=None):
    ...
    if len(parts) > 2:
        ...
        print("Single part name, using directly")
...
def resolve_table_name(name: str, ctx=None):
    ...
    # # Handle ACL stage
    # if len(parts) >= 3 and parts[1] == "acl" and parts[2].startswith("stage"):
    #     direction, _, stage = parts[:3]
    #     var_name = f"{direction}_acl_{stage}"
    #     print(f"Detected ACL stage pattern: {var_name}")
    #     if var_name in ctx:
    #         print(f"Resolved to variable in context: {var_name}")
    #         return ctx[var_name]
    ...
    for i, part in enumerate(parts, start=1):
        try:
            ...
            print(f"[resolve_table_name] ERROR: cannot resolve '{part}' - {e}")
            return None
    return obj
