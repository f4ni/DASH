...
class conntrack_flow_miss_handle():
    ...
    def apply(cls):
        ...
        # SYN
        # if (hdr.customer_tcp.isValid() and hdr.customer_tcp.flags == 0x2) or hdr.customer_udp.isValid():
...
class conntrack_flow_handle():
    @classmethod
    def apply(cls):
        py_log("info", "conntrack_flow_handle called")
...
class conntrack_lookup_stage:
    ...
    def apply(cls):
        ...
        if hdr.flow_key is None:
            py_log("info", "Lookup in 'flow_table' ")
        ...
        py_log("info", "Lookup in 'flow_entry' ")
        cls.flow_entry.apply()
        py_log("info", "Lookup in 'flow_entry_bulk_get_session_filter'")
        cls.flow_entry_bulk_get_session_filter.apply()
        py_log("info", "Lookup in 'flow_entry_bulk_get_session'")
