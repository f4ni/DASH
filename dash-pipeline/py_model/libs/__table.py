...
def TERNARY(entry_value: Entry.Ternary, match_value: int, width: int):
    ...
    py_log(None, f"TERNARY value={hex(val)} mask={hex(mask)} match={hex(match_value)}")
    return (val & mask) == (match_value & mask)

def LIST(entry_value: list[Entry.Ternary], match_value: int, width: int):
    for ev in entry_value:
        py_log(None, f"TERNARY LIST ev={ev} match_value={hex(match_value)}")
        if TERNARY(ev, match_value, width):
            return True
    return False

def RANGE(entry_value: Entry.Range, match_value: int, width: int):
    py_log(None, f"RANGE value={match_value} low={entry_value.low} high={entry_value.high}")
    return entry_value.low <= match_value <= entry_value.high

def RANGE_LIST(entry_value: list[Entry.Range], match_value: int, width: int):
    for ev in entry_value:
        py_log(None, f"RANGE LIST ev={ev} match_value={match_value}")
...
class Table:
    ...
    def apply(self):
        ...
        py_log("info", f"Match {'FOUND' if entry else 'NOT FOUND'} | Action: {action.__name__}{params}\n")
        action(*params)
    ...
    def __match_entry(self, entry: Entry):
        for idx, (k, match_routine) in enumerate(self.key.items()):
            ...
            result = match_routine(entry_value, match_value, width)
